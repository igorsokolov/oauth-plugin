...
#         if token = Oauth2Token.where(:invalidated_at => nil).where('authorized_at IS NOT NULL').where(:token => token_string).first
          if token = Oauth2Token.where(:invalidated_at => nil).where(:token => token_string).first
